...
@Slf4j
@Component(value = "VerifyIfOptionWithIndexIsSelectedInListBox")
public class VerifyIfOptionWithIndexIsSelectedInListBox implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            log.info("Verifying ListBox if option with index " + index + " is selected in " + elementName + " " + elementType);
            ...
            if (index < allOptions.size()) {
                ...
                if (option.isSelected()) {
                    log.info("Option with index " + index + " is selected in " + elementName + " " + elementType);
                    optionIsSelected = true;
                    nlpResponseModel.setMessage(modifiedPassMessage);
                    nlpResponseModel.setStatus(CommonConstants.pass);
                } else {
                    log.error("Option with index " + index + " is not selected in " + elementName + " " + elementType);
                    nlpResponseModel.setMessage(modifiedFailMessage);
                    nlpResponseModel.setStatus(CommonConstants.fail);
                    nlpResponseModel.setIfCheckPointIsFailed(ifCheckPointIsFailed);
                }
            } else {
                log.error("Invalid index is provided");
                nlpResponseModel.setMessage(modifiedFailMessage);
                nlpResponseModel.setStatus(CommonConstants.fail);
                nlpResponseModel.setIfCheckPointIsFailed(ifCheckPointIsFailed);
            }
        } catch (Exception exception) {
            log.error("NLP_EXCEPTION in VerifyIfOptionWithIndexIsSelectedInListBox ", exception);
        ...
        nlpResponseModel.getAttributes().put("optionIsSelected", optionIsSelected);
    ...
    public StringBuilder getTestCode() throws NlpException {
        ...
        sb.append("			if (textOfAllSelectedOptions.get(j).equals(option) && i == index) {\n");
        sb.append("				flag = true;\n");
        sb.append("				break;\n");
        sb.append("			}\n");
        sb.append("	}\n");
        ...
        sb.append("	} else {\n");
        sb.append("		System.out.println(\"Option with Index \" + index + \" is not selected\");\n");
        sb.append("	}\n");
        sb.append("} else {\n");
