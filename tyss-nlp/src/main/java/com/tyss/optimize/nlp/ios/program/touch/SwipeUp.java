...
@Slf4j
@Component(value = "IOS_SwipeUp")
public class SwipeUp implements Nlp {
    @Override
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            log.info("Swiping from bottom to top");
            if (fromY > toY) {
                new Swipe().execute(requestSwipe);
                log.info("Swiped screen from bottom to top");
                nlpResponseModel.setMessage(requestSwipe.getPassMessage());
                nlpResponseModel.setStatus(CommonConstants.pass);
            } else {
                log.error("Entered coordinate value is wrong");
                log.error("Failed to swipe screen from bottom to top");
                nlpResponseModel.setMessage(requestSwipe.getFailMessage());
            ...
            log.error("NLP_EXCEPTION in SwipeUp ", exception);
    ...
    public StringBuilder getTestCode() throws NlpException {
        ...
        sb.append("	 System.out.println(\"Swiped screen from bottom to top\");\n");
        sb.append("} else {\n");
        sb.append("	 System.out.println(\"Entered coordinate value is wrong\");\n");
        sb.append("	 System.out.println(\"Failed to swipe screen from bottom to top\");\n");
        sb.append("}\n");
