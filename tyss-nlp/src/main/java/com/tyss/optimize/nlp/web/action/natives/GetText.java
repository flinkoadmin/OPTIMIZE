...
public class GetText implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            log.info("Getting the size of " + elementName + " " + elementType);
            text = element.getText();
            String modifiedPassMessage = passMessage.replace("*elementName*", elementName)
                    .replace("*elementType*", elementType).replace("*returnValue*", String.valueOf(text));
           log.info("modified pass message "+modifiedPassMessage);
            nlpResponseModel.setMessage(modifiedPassMessage);
            nlpResponseModel.setStatus(CommonConstants.pass);
        } catch (Exception exception) {
            log.error("NLP_EXCEPTION in GetText ", exception);
