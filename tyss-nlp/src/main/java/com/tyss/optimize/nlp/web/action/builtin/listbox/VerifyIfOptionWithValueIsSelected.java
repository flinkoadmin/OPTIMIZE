...
@Slf4j
@Component(value = "VerifyIfOptionWithValueIsSelected")
public class VerifyIfOptionWithValueIsSelected implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            if (match) {
                ...
                if (flag) {
                    log.info("Option with value " + value + " is selected in " + elementName + " " + elementType);
                    isSelected = true;
                    nlpResponseModel.setMessage(modifiedPassMessage);
                    nlpResponseModel.setStatus(CommonConstants.pass);
                } else {
                    log.error("Option with value " + value + " is not selected in " + elementName + " " + elementType);
            ...
            log.error("NLP_EXCEPTION in VerifyIfOptionWithValueIsSelected ", exception);
    ...
    public StringBuilder getTestCode() throws NlpException {
        ...
        sb.append("	}\n");
        sb.append("	if (flag) {\n");
        sb.append("		System.out.println(\"Option with Value \" + value + \" is selected\");\n");
        sb.append("	} else {\n");
        sb.append("		System.out.println(\"Option with Value \" + value + \" is not selected\");\n");
        sb.append("	}\n");
        sb.append("} else {\n");
